...
class MapfmState:
    def __init__(self, agents: Iterator[Agent], new_agents=None, accumulated_cost: Optional[List[int]] = None, new_accumulated_cost=None):
    ...
    def get_next(self) -> Tuple[int, Agent, int]:
        """
        Returns the next agent without a move as well as its index and its previous accumulated cost
        """
        i = len(self.new_agents)
        return i, self.agents[i], self.accumulated_cost[i]

    def move_with_agent(self, agent, acc_cost) -> MapfmState:
        """
