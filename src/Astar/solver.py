...
class Solver:
    ...
    def solve(self) -> Optional[List[AgentPath]]:
        ...
        while frontier:
            ...
            if popped % 100000 == 0:
                print(f"Solver:     Count: {popped}, Heuristic: {current.heuristic}, Cost: {current.cost}, F: {current.cost + current.heuristic}, Frontier size: {len(frontier)}, Max: {self.max_cost}")
